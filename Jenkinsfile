    ...
    stages {
        stage('Login, Build and Push'){
            steps {
                script{
                    ...
                    withDockerRegistry(credentialsId: 'Joonbuggz') {
        ...
        stage('AWS Commands'){
            steps {
                script {
                    // sign into AWS
                    withAWS(credentials: 'Docker', region: 'us-east-1'){ 
                ...
                script {
                    withAWS(credentials: 'Docker', region: 'us-east-1') {
                        try {
                            ...
                            sh 'kubectl rollout restart deployment flask-deployment -n jbuggz-namespace'
                        } catch (Exception e) {
